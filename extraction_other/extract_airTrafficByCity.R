...
#============ Relevant airports/countries to extract air trafic from
...
if (file.exists("extraction_other/data_airTrafficByCity_ts.rds")) {
  historicDailyFlights <- readRDS("extraction_other/data_airTrafficByCity_ts.rds")
...
if (length(unProcessedDates) > 0) {
  ...
  # ================================
  # # Save dataset
  saveRDS(as_tibble(historicDailyFlights),"extraction_other/data_airTrafficByCity_ts.rds")
  ...
  geo_airTraffic
  saveRDS(geo_airTraffic,"extraction_other/geo_airTrafficByCity.rds")
}