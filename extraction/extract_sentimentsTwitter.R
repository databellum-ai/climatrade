# PTE: revisar filtros ¿por qué no hay nada de @POTUS?
...
# PTE: sacar inicialización de las listas de palabras del loop
...
# ================================
# STEP 1: Extract texts from Twitter
# ================================
...
for (i in c(1:length(searchTerms))) {
  ...
  tweets_found <- 
    searchTwitter(searchTerms[i], n = 1000, resultType = "recent", lang = "en", retryOnRateLimit = 100)
  ...
  tweets_found.nodups.df <- tweets_found.nodups.df %>% 
    mutate(created = as.Date(tweets_found.nodups.df$created)) %>% rename(date = created)
...
scores.df <- scores.df %>% spread(key = term, value = score, fill=NA)
head(scores.df)
