...
selected_initial_year_OECD <- "2010"
...
leadingIndicatorsOECD %>%
  ...
  group_by(Indicator, obsDate) %>% 
  summarize(Valor = mean(obsValue)) %>% 
  ggplot(aes(as_date(obsDate), Valor, color=Indicator)) + geom_line()
