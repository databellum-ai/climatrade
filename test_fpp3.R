...
df_planetMood_ts <- df_planetMood_1 %>% as_tsibble(index = date)  # convert to tsibble (time serie)
df_planetMood_train_ts <- df_planetMood_train %>% as_tsibble(index = date)
futureData_ts <- as_tsibble(df_planetMood_1 %>% 
                              select(c(-VIX, -selectedVbles)) %>% 
                              filter(date >= firstDateToForecast & date <= lastDateToForecast))

# Available datasets
head(df_planetMood_1)
...
allVblesMosaic <- allVbles[c(-1,-32)]
n <- length(allVblesMosaic)
...
for(i in 1:n) {
  matplot(df_planetMood[,allVblesMosaic][,i], axes=FALSE,
          type=c('l'), col = c('blue'), 
          main = names(df_planetMood[,allVblesMosaic])[i])
  axis(2) # show y axis
